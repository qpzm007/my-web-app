...
function useSupabaseSync<T extends { id: string }>(tableName: string, initialData: T[] = []): [T[], React.Dispatch<React.SetStateAction<T[]>>, boolean] {
  ...
  useEffect(() => {
    if (!hasSupabase) {
      // No Supabase configured — use initial local data only
  ...
  const setSyncData: React.Dispatch<React.SetStateAction<T[]>> = (action) => {
    setData((prev) => {
      ...
      if (isLoadedRef.current && hasSupabase) {
        setTimeout(async () => {
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [projects, setProjects] = useSupabaseSync<Project>('projects', INITIAL_PROJECTS);
  const [allProcesses, setAllProcesses] = useSupabaseSync<FactoryProcess>('processes', INITIAL_PROCESSES);
  const [allWorkers, setAllWorkers] = useSupabaseSync<Worker>('workers', INITIAL_WORKERS);
  ...
  if (hasSupabase && !projects.length) {
